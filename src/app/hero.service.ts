...
export class HeroService {
  ...
  getHerwithout404(id: number): Observable<Hero> {
  ...
  searchHeroesByName(term: string): Observable<Hero[]> {
    ...
    searchHeroesByPower(term: string): Observable<Hero[]> {
      ...
      return this.http.get<Hero[]>(`${this.heroesUrl}/?power=${term}`).pipe(
        tap(x => x.length ?
          this.log(`found heroes with power matching "${term}"`) :
          this.log(`no heroes with power matching "${term}"`)),
        catchError(this.handleError<Hero[]>('searchHeroesByPower', []))
      );
    }
